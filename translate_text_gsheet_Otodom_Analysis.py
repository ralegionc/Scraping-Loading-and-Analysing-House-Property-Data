...
with engine.connect() as conn:
    try:
        query = """ SELECT RN, TITLE FROM otodom_data_short_flatten ORDER BY rn limit 300"""

        df = pd.read_sql(query,conn)

        gc=gspread.service_account(filename= r"filelocation")

        loop_counter = 0
        chunk_size = 100
        file_name = 'OTODOM_ANALYSIS_'
        user_email = 'workfar33@gmail.com'
